...
function get_final_price($goods_id, $goods_num = '1', $is_spec_price = false, $spec = array())
{
	...
	//增加团购价格 qxfz.cn
	$group_buy = group_buy_info2($goods_id,$goods_num);  
	$group_buy_price = $group_buy['cur_price'];   
	if (!empty($group_buy_price))
	{
		$final_price = min($group_buy_price, $final_price);
	}
...
$href ='技术支持：<a title="Ecshop模板网" target="_blank" href="http://www.ecshoptemplate.com/">Ecshop模板网</a>';
if(!strstr($content,$href)){
echo "您好，当您看到这些文字的时候，是因为你删除了网站底部ECSHOP模板网模板授权文字和连接：（技术支持：Ecshop模板网）导致的，请您添加回去,谢谢合作！如需帮助联系最模板，网址：http://www.zuimoban.com";
exit();
}
}
