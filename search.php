...
else
{
    ...
    $sort  = (isset($_REQUEST['sort'])  && in_array(trim(strtolower($_REQUEST['sort'])), array('goods_id', 'shop_price', 'sell_number'))) ? trim($_REQUEST['sort'])  : $default_sort_order_type;   //增加销售量排序qxfz.cn
    ...
    while ($row = $db->FetchRow($res))
    {
		...
		$arr[$row['goods_id']]['comment_count']    = $row['comment_count'];   //qxfz.cn	
		$arr[$row['goods_id']]['comment_rank']  = ceil($row['comment_rank']) == 0 ? number_format(5, 2, '.', '') : number_format($row['comment_rank'], 2, '.', '');
		$arr[$row['goods_id']]['comment_rank_int']  = ceil($row['comment_rank']) == 0 ? 5 : ceil($row['comment_rank']);
