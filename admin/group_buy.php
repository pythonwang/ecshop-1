...
include_once(ROOT_PATH . 'includes/cls_image.php');  //qxfz.cn
...
elseif ($_REQUEST['act'] =='insert_update')
{
	...
	//图片 qxfz.cn
    if (isset($_FILES['act_img']) && $_FILES['act_img']['tmp_name'] != '' &&
        isset($_FILES['act_img']['tmp_name']) &&$_FILES['act_img']['tmp_name'] != 'none')
    {
	   $act_img = basename($image->upload_image($_FILES['act_img'],'group_buy'));
	}
	...
	{
    /* 删除原来的图片 */
    $sql = "SELECT act_img FROM " .$ecs->table('goods_activity'). " WHERE act_id = '$group_buy_id'";
    $act_oldimg = $db->getOne($sql);
     if (!empty($act_oldimg))
     {
        @unlink(ROOT_PATH . DATA_DIR . '/group_buy/' .$act_oldimg);
     }
	}
