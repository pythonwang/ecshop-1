...
elseif ($_REQUEST['act'] == 'buy')
{
	...
	$goods_id = isset($_GET['goods_id']) ? intval($_GET['goods_id']) : 0;      //Get获取  qxfz.cn
    if ($goods_id <= 0)
    {
        ecs_header("Location: ./\n");
        exit;
...
function exchange_get_goods($children, $min, $max, $ext, $size, $page, $sort, $order)
{
    ...
    $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style, eg.exchange_integral, ' .
                'g.goods_type, g.goods_brief, g.goods_thumb , g.goods_img, eg.is_hot,g.shop_price,g.promote_price ' .
...
//        else
		...
		$arr[$row['goods_id']]['gurl']         = build_uri('goods', array('gid'=>$row['goods_id']), $row['goods_name']);   //qxfz.cn
